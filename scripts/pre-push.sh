#!/usr/bin/env bash
# Pre-push hook for KTPCvarChecker.
#
# This repo is a ktp_discord.inc consumer (no direct curl calls today;
# defense-in-depth for future curl additions). Ships an AMXX plugin loaded
# on every production KTP DoD server. The hook gates pushes with the
# canonical amxxcurl async-lifetime lint from KTPInfrastructure — catches
# the slist UAF and sync-cleanup-after-async-perform anti-patterns
# documented in KTPAmxxCurl commit 7e1ce00 (NY1 outage 2026-04-26).
#
