...
- **`ex_interp` below the floor never blocks (v7.42)** — a sub-floor `ex_interp` is corrected and continues: the player is told once (`INTERP_UNCORRECTED`), the floor stays `0.01`, and the blocked state that refuses `.ready` is never entered. `0.009` was the floor itself until v7.38, so the value was legal and widespread before it became a violation. The exemption is that cvar under that bound only — every other cvar, and `ex_interp` above its ceiling, still blocks
