...
**Version 7.34** - Priority-based client cvar enforcement for competitive Day of Defeat servers.
...
- **Netcode observation (v7.34)** — `rate` and `cl_updaterate` are read from userinfo (never queried, never enforced) and logged on change as `NETOBS_CHANGED`, with a once-per-map `NETOBS_SAMPLER_OK` liveness line. `NETOBS_INTERP_LOW` fires when `ex_interp` is below `1/cl_updaterate` — the enforced ranges (`cl_updaterate` 100-120, `ex_interp` 0.009-0.05) let a client satisfy both rules and still extrapolate, because until now each cvar was only ever validated on its own. An extrapolating client is the one other players see warping
