...
**Version 7.42** - Priority-based client cvar enforcement for competitive Day of Defeat servers.
...
- **Blocked-player bridge (v7.41)** — `ktp_cvar_get_blocked` (library `ktp_cvar_checker`) reports a player still blocking a correction, and KTPMatchHandler refuses their `.ready` for a match. The state clears when the cvar answers in range or the player leaves
- **`ex_interp` below the floor never blocks (v7.42)** — a sub-floor `ex_interp` is corrected like any other violation and then left alone instead of escalating: the floor stays `0.01`, and the blocked state that refuses `.ready` is never entered. Corrections stop at `MAX_ENFORCE_ATTEMPTS` exactly as the blocked path does, and the player is told once per streak (`INTERP_UNCORRECTED`) — a streak restarts at each map change, so a client that filters stuffcmds keeps playing sub-floor, uncorrected and unblocked, with that line as the only record. `0.009` was the floor itself until v7.38, so the value was legal and widespread before it became a violation. The exemption is that cvar under that bound only — every other cvar, and `ex_interp` above its ceiling, still blocks
