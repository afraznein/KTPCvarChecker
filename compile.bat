...
echo Compiling ktp_cvar.sma via WSL...
echo.

:: Compile the plugin using WSL (include AMXX, ReAPI, and ReAPI version paths)
wsl bash -c "cd '%SCRIPT_DIR%' && '%WSL_AMXXPC%' ktp_cvar.sma -i'%WSL_INCLUDE%' -i'%WSL_REAPI_INCLUDE%' -i'%WSL_REAPI_VERSION%' -o'%WSL_OUTPUT%/ktp_cvar.amxx'"
