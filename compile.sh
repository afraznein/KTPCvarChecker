#!/bin/bash
# KTPCvarChecker Plugin Compiler - WSL/Linux version
...
_ktp_build_exit() {
    ...
    if [ "$rc" -ne 0 ]; then
        echo ""
        echo "========================================"
        echo "[KTP-BUILD] FAILED: KTPCvarChecker compile.sh exited $rc"
        echo "========================================"
...
echo "========================================"
echo "KTPCvarChecker Plugin Compiler (WSL)"
echo "========================================"
echo
...
if [ -n "${BASH_SOURCE[0]}" ] && [ -f "${BASH_SOURCE[0]}" ]; then
    ...
    SCRIPT_DIR="/mnt/n/Nein_/KTP Git Projects/KTPCvarChecker"
fi
PLUGIN_NAME="ktp_cvar"
...
# into the .amxx and reported by `amx_ktp_versions` over rcon, so an artifact that
# bakes "unknown" cannot say where it came from -- and nothing in the build output
# used to say so.
#
...
# `|| echo "unknown"` swallowed it, and GIT_DIRTY below is only computed when the
# SHA resolves -- so the result was indistinguishable from a legitimate clean
# off-toolchain build.
#
...
# must not ship without provenance. Neither is set by default, so an ordinary
# off-toolchain build still works and just says what it is doing.
...
echo "[KTP-BUILD] OK: KTPCvarChecker compile.sh"
